...
# parser.add_argument("--valid_coords_seq_hdf5", type=str, default="./", help="hdf5 path for coords and seqs data of validation set")
parser.add_argument("--train_list_dir", type=str, default="./", help="path for train sample list file")
...
for epoch in range(FLAGS.epoch_start, FLAGS.epoch_end + 1):
    ...
    # different sample list for different epoch
    train_set = []
    with open(os.path.join(FLAGS.train_list_dir, f"epoch_{epoch}.txt"), "r") as FILE:
        for line in FILE:
            sample_name = re.split("\t", line.strip())[0]
            train_set.append(sample_name)
        ...
        if sample_index % 30000 == 0 or sample_index == FLAGS.train_sample_num:
            ...
            with torch.no_grad():  # no gradient when validation
                ...
                for i in valid_index:
                    ...
                    labelall, psi_phi, f1d, f2d, target_tokens, target_seq, true_xyz = getDataValid(protein_name, PDB_coords_seq_hdf5, esm2_CPU, esm2_GPU, batch_converter_esm2, device_esm, device1_SPIRED, device2_SPIRED, FLAGS)
                    ...
                    if FLAGS.write_pdb_num > 0:
                        ...
                        xyz_topK = Predxyz["4th"][-1][0, :, plddt_topK_idx, :].permute(1, 2, 0)  # (N,L,3)
