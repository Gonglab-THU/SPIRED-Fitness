...
for epoch in range(FLAGS.epoch_start, FLAGS.epoch_end + 1):
    ...
    for i in range(0, FLAGS.train_sample_num):
        ...
        s0 = time.time()
        if re.search("_", protein_name):  # PDB data
        ...
        elif re.search("[a-z]", protein_name) and len(protein_name) == 7:  # CATH data
            (label_batch, psi_phi_batch, f1d_batch, f2d_batch, target_tokens, true_xyz) = getDataTrain(protein_name_batch, CATH_coords_seq_hdf5, FLAGS.maxlen, esm2_CPU, esm2_GPU, batch_converter_esm2, device_esm, device1_SPIRED, device1_SPIRED, FLAGS)
