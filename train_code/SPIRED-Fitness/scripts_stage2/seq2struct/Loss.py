...
def tmscore_torch(coords1, true_coords, dcut=15.0):
    # coords1 = (N,L,3) CA coords
    # coords2 = (L,3) CA coords
    N, L, _ = coords1.shape
...
def lossViolation(predcadist):
    ...
    masklocal = torch.as_tensor(torch.abs(a - a.T) >= 2, dtype=torch.float32)
...
def getFapeLoss(diff, coord_idx, dclamp, ratio=0.1, lossformat="dclamp"):
    ...
    if lossformat == "dclamp":
        ...
        diff = ratio * nn.ReLU()(diff - dclamp) + dclamp - nn.ReLU()(dclamp - diff)  # 考虑换成mask形式
        ...
        maskprob = torch.as_tensor(torch.rand([L, L]) >= (1 - ratio), dtype=torch.float32)  # uniform in [0,1), 按照概率ratio提取dclamp之外的残基对
...
# xtx soft spearman loss for fitness prediction
