...
class BlockV0(nn.Module):
    ...
    def forward(self, seq_feats, pair_feats, true_aa, residx, mask, maskdiag):
        ...
        pair_feats = self.forNextBlock(pair_feats, cadistavg)  # 保持原状，有需要时再做对称化
...
class SPIRED_Model(nn.Module):
    def __init__(self, depth=2, channel=128, device_list=["cuda:0", "cuda:1", "cuda:2", "cuda:3"]):
        ...
        ######
        # print("device", self.device0, self.device1, self.device2, self.device3)
    ...
    def forward(self, true_aa, f1d, no_recycles):
        ...
        if self.esm_s_combine["weight"].device == torch.device("cpu"):
            print("on CPU")
            self.device0, self.device1, self.device2, self.device3 = (torch.device("cpu"), torch.device("cpu"), torch.device("cpu"), torch.device("cpu"))

        # print("f1d", f1d.shape, "true_aa", true_aa.shape, "no_recycles", no_recycles, self.esm_s_combine["weight"].softmax(0).unsqueeze(0).shape)
        ...
        # print("f1d", f1d.device, "self.esm_s_combine", self.esm_s_combine["weight"].device)
        ...
        for recycle_idx in range(no_recycles):
            with ExitStack() if recycle_idx == (no_recycles - 1) else torch.no_grad():
                ...
                print(f"plddt_max {recycle_idx}", plddt_max)
...
class SPIRED_Fitness_Union(torch.nn.Module):
    def __init__(self, device_list):
        ...
        self.Fitness_device = device_list[-1]
        self.Fitness = PretrainModel(node_dim=32, num_layer=2, n_head=8, pair_dim=32).to(self.Fitness_device)
    ...
    def forward(self, true_aa, f1d_esm2_3B, f1d_esm2_650M, esm1v_single_logits, esm1v_double_logits):
        ...
        xyz_LL3 = Predxyz["4th"][-1].permute(0, 2, 3, 1).contiguous()  # (batch=1,L,L,3), Predxyz_4thBlock_6thlayer
        plddt_value = Plddt["4th"][-1][0].unsqueeze(0)  # (1, L, L)
