...
def getDataTrain(domainbatch, trainhdf5, esm2_3B_hdf5, fitness_pt, Lmax, batch_converter_esm2, device1, device2, FLAGS):

    Length = []
    featuredict = {}
    labeldict = {}
    domain = domainbatch[0]

    s0 = time.time()

    for domain in domainbatch:
        target_tokens = trainhdf5[domain]["target_tokens"][:]  # (1,L)
        Length.append(target_tokens.shape[-1])

        xyz = trainhdf5[domain]["xyz"][:]  # (L,4,3) [N, CA, C, CB]
        f1d_esm2_3B = torch.from_numpy(esm2_3B_hdf5[domain]["f1d_esm2_3B"][:])  # (1,L,37,2560)
        psi_phi = trainhdf5[domain]["Psi_Phi"][:]  # (L-1, 2) [psi, phi]

        f1d_esm2_650M = torch.from_numpy(fitness_pt[domain]["1d"][:])  # (L, 1280)
        esm1v_single_logits = torch.from_numpy(fitness_pt[domain]["single_logits"][:])  # (5, L, 20)
        esm1v_double_logits = torch.from_numpy(fitness_pt[domain]["double_logits"][:])  # (5, L, 20)
        single_index = fitness_pt[domain]["single_train"][:].tolist()  # (2,) List
        double_index = fitness_pt[domain]["double_train"][:].tolist()  # (2,) List
        single_label = torch.from_numpy(fitness_pt[domain]["single_label"][:])  # (L, 20)
        double_label = torch.from_numpy(fitness_pt[domain]["double_label"][:])  # (L, 20)

        # labelall = torch.from_numpy(trainhdf5[domain]['labelall'][:]) # (8, L, L)
        labeldict[domain] = [xyz, psi_phi, single_label, double_label]
        featuredict[domain] = [target_tokens, f1d_esm2_650M, f1d_esm2_3B, esm1v_single_logits, esm1v_double_logits]

    Lmin = np.min(Length)
    if Lmin <= Lmax:
        for i in range(len(Length)):
            domain = domainbatch[i]
            target_tokens, f1d_esm2_650M, f1d_esm2_3B, esm1v_single_logits, esm1v_double_logits = featuredict[domain]
            xyz, psi_phi, single_label, double_label = labeldict[domain]

            if Length[i] > Lmin:
                L = Length[i]
                beg = np.random.randint(L - Lmin + 1)  # random int in [0,L-Lmax+1)
                target_tokens = target_tokens[:, beg : beg + Lmin]
                f1d_esm2_650M = f1d_esm2_650M[beg : beg + Lmin, :]
                f1d_esm2_3B = f1d_esm2_3B[:, beg : beg + Lmin, :, :]
                esm1v_single_logits = esm1v_single_logits[:, beg : beg + Lmin, :]
                esm1v_double_logits = esm1v_double_logits[:, beg : beg + Lmin, :]
                xyz = xyz[beg : beg + Lmin, :, :]
                psi_phi = psi_phi[beg : beg + Lmin - 1, :]
                single_label = single_label[beg : beg + Lmin, :]
                double_label = double_label[beg : beg + Lmin, :]
                # labelall = labelall[:,beg:beg+Lmin,beg:beg+Lmin]

            featuredict[domain] = [target_tokens, f1d_esm2_650M, f1d_esm2_3B, esm1v_single_logits, esm1v_double_logits]
            labelall = comp_feature(xyz)  # (8,L,L)
            labeldict[domain] = [labelall, psi_phi, xyz, single_label, double_label]
    else:
        for domain in domainbatch:
            target_tokens, f1d_esm2_650M, f1d_esm2_3B, esm1v_single_logits, esm1v_double_logits = featuredict[domain]
            xyz, psi_phi, single_label, double_label = labeldict[domain]
            ...
            f1d_esm2_650M = f1d_esm2_650M[beg : beg + Lmax, :]
            f1d_esm2_3B = f1d_esm2_3B[:, beg : beg + Lmax, :, :]
            esm1v_double_logits = esm1v_double_logits[:, beg : beg + Lmax, :]
            esm1v_single_logits = esm1v_single_logits[:, beg : beg + Lmax, :]
            xyz = xyz[beg : beg + Lmax, :, :]
            psi_phi = psi_phi[beg : beg + Lmax - 1, :]
            single_label = single_label[beg : beg + Lmax, :]
            double_label = double_label[beg : beg + Lmax, :]
    ...
    token_list, f1d_esm2_650M_list, f1d_esm2_3B_list, esm1v_single_logits, esm1v_double_logits, single_label_list, double_label_list, label_8LL_list, psi_phi_list, xyz_list = ([], [], [], [], [], [], [], [], [], [])
    for domain in domainbatch:
        token_list.append(featuredict[domain][0])
        f1d_esm2_650M_list.append(featuredict[domain][1].unsqueeze(0))  # (1,L,1280)
        f1d_esm2_3B_list.append(featuredict[domain][2])  # (1, L, 37, 2560)
        esm1v_single_logits.append(featuredict[domain][3].unsqueeze(0))  # (1,5,L,20)
        esm1v_double_logits.append(featuredict[domain][4].unsqueeze(0))  # (1,5,L,20)

        label_8LL_list.append(labeldict[domain][0].unsqueeze(0))  # (1,8,L,L)
        ...
        single_label_list.append(labeldict[domain][3].unsqueeze(0))  # (1,L,20)
        double_label_list.append(labeldict[domain][4].unsqueeze(0))  # (1,L,20)

    label_8LL_batch = torch.cat(label_8LL_list, dim=0).to(device2)  # (batch, 8, L, L)
    psi_phi_batch = torch.cat(psi_phi_list, dim=0).to(device2)  # (batch, 2, L)
    xyz_batch = torch.cat(xyz_list, dim=0).to(device2)  # (batch, L,4,3)
    single_label_bacth = torch.cat(single_label_list, dim=0).to(device2)  # (batch, L, 20)
    double_label_bacth = torch.cat(double_label_list, dim=0).to(device2)  # (batch, L, 20)
    f1d_esm2_650M_bacth = torch.cat(f1d_esm2_650M_list, dim=0).to(device2)  # (1,L,1280)
    f1d_esm2_3B_bacth = torch.cat(f1d_esm2_3B_list, dim=0).to(device2)  # (1,L,1280)
    esm1v_single_logits_batch = torch.cat(esm1v_single_logits, dim=0).to(device2)  # (1,5,L,20)
    esm1v_double_logits_batch = torch.cat(esm1v_double_logits, dim=0).to(device2)  # (1,5,L,L,400)
    print("single_label_bacth", single_label_bacth.shape, "f1d_esm2_650M_bacth", f1d_esm2_650M_bacth.shape, "esm1v_single_logits_batch", esm1v_single_logits_batch.shape)
    ...
    # f1d_esm2_3B, f2dbatch = ESM2_embed(target_tokens, batch_converter_esm2, model_CPU, model_GPU, length_cutoff = FLAGS.ESM_length_cutoff, gpu=device_esm) # f1dbatch=(N,L,C), f2dbatch=(N,C,L,L)
    # f1d_esm2_3B = f1d_esm2_3B.to(device1)

    s2 = time.time()
    t_esm2 = s2 - s1
    print("t_esm2", t_esm2)

    return (single_label_bacth, double_label_bacth, single_index, double_index, label_8LL_batch, psi_phi_batch, f1d_esm2_3B_bacth, f1d_esm2_650M_bacth, esm1v_single_logits_batch, esm1v_double_logits_batch, target_tokens[:, 1:-1], xyz_batch)


def getData_fitness(train_test, domain, hdf5, esm2_3B_hdf5, fitness_pt, batch_converter_esm2, device1, device2, FLAGS):
    s0 = time.time()
    target_tokens = hdf5[domain]["target_tokens"][:]  # (1,L)
    L = target_tokens.shape[-1]
    if L > 800:
        print("length", L, "CPU")
        device1, device2 = (torch.device("cpu"), torch.device("cpu"))

    xyz = hdf5[domain]["xyz"][:]  # (L,4,3) [N, CA, C, CB]
    label_8LL = comp_feature(xyz)  # (8,L,L)
    psi_phi = hdf5[domain]["Psi_Phi"][:]  # (L-1, 2) [cos(psi), cos(phi)]

    f1d_esm2_650M = torch.from_numpy(fitness_pt[domain]["1d"][:])  # (L, 1280)
    esm1v_single_logits = torch.from_numpy(fitness_pt[domain]["single_logits"][:])  # (5, L, 20)
    esm1v_double_logits = torch.from_numpy(fitness_pt[domain]["double_logits"][:])  # (5, L, L, 400)
    single_index = fitness_pt[domain][f"single_{train_test}"][:].tolist()  # (2,) List
    double_index = fitness_pt[domain][f"double_{train_test}"][:].tolist()  # (2,) List
    single_label = torch.from_numpy(fitness_pt[domain]["single_label"][:])  # (L, 20)
    double_label = torch.from_numpy(fitness_pt[domain]["double_label"][:])  # (L, L, 400)
    seq_str = tokens2seq(target_tokens)
    ...
    f1d_esm2_3B = f1d_esm2_3B.to(device1)  # (N,L,37,2560)
    label_8LL = label_8LL.to(device2).unsqueeze(0)  # (1, 8, L, L)
    psi_phi = torch.tensor(psi_phi).permute(1, 0).unsqueeze(0).to(device2)  # (1, 2, L)
    xyz = torch.tensor(xyz).unsqueeze(0).to(device2)  # (1,L,4,2)

    f1d_esm2_650M = f1d_esm2_650M.unsqueeze(0).to(device1)  # (1, L, 1280)
    esm1v_single_logits = esm1v_single_logits.unsqueeze(0).to(device1)  # (1, 5, L, 20)
    esm1v_double_logits = esm1v_double_logits.unsqueeze(0).to(device1)  # (1, 5, L, L 400)
    single_label = single_label.unsqueeze(0).to(device2)  # (1, L, 20)
    double_label = double_label.unsqueeze(0).to(device2)  # (1, L, L, 400)

    # print("single_label", single_label.shape, "f1d_esm2_650M", f1d_esm2_650M.shape, "esm1v_single_logits", esm1v_single_logits.shape)

    s2 = time.time()
    t_label = s2 - s1
    print("t_label", t_label)

    return (single_label, double_label, single_index, double_index, label_8LL, psi_phi, f1d_esm2_3B, f1d_esm2_650M, esm1v_single_logits, esm1v_double_logits, target_tokens[:, 1:-1], seq_str[0][1], xyz)


def getData_PDB(domain, hdf5, esm2_3B_CPU, esm2_3B_GPU, batch_converter_esm2, device_esm, device1, device2, length_cutoff, FLAGS):
    s0 = time.time()
    target_tokens = hdf5[domain]["target_tokens"][:]  # (1,L)
    L = target_tokens.shape[-1]
    ...
    if L > length_cutoff:
        target_tokens = target_tokens[:, :length_cutoff]
        xyz = xyz[:length_cutoff, :, :]
        psi_phi = psi_phi[: length_cutoff - 1, :]

    label_8LL = comp_feature(xyz)  # (8,L,L)
    seq_str = tokens2seq(target_tokens)
    labels, strs, target_tokens = batch_converter_esm2(seq_str)

    f1d_esm2_3B, f2d = ESM2_embed(target_tokens, batch_converter_esm2, esm2_3B_CPU, esm2_3B_GPU, length_cutoff=FLAGS.ESM_length_cutoff, gpu=device_esm)
    s1 = time.time()
    t_esm = s1 - s0
    print("t_esm", t_esm)
    ...
    L = label_8LL.shape[-1]
    f1d_esm2_650M = torch.randn(1, L, 1280).to(device1)  # (1, L, 1280)
    esm1v_single_logits = torch.randn(1, 5, L, 20).to(device1)  # (1, 5, L, 20)
    esm1v_double_logits = torch.randn(1, 5, L, L, 400).to(device1)  # (1, 5, L, L, 400)
    single_label = torch.randn(1, L, 20).to(device2)  # (1, L, 20)
    double_label = torch.randn(1, L, L, 400).to(device2)  # (1, L, L, 400)
    single_index = [[], []]
    double_index = [[], [], []]
...
def gen_esm1v_logits(model, batch_converter, alphabet, seq_data):
...
def getDataTest(seq_data, ESM2_3B, ESM2_650M, ESM1v_1, ESM1v_2, ESM1v_3, ESM1v_4, ESM1v_5, batch_converter_esm2, alphabet, device1):
    labels, strs, target_tokens = batch_converter_esm2(seq_data)

    with torch.no_grad():
        results = ESM2_3B(target_tokens.to(device="cpu"), repr_layers=range(37), need_head_weights=False, return_contacts=True)
        f1d_esm2_3B = torch.stack([v for _, v in sorted(results["representations"].items())], dim=2)
        f1d_esm2_3B = f1d_esm2_3B[:, 1:-1]  # (batch, L, dim=2560)
        f1d_esm2_3B = f1d_esm2_3B.to(device=device1, dtype=torch.float32)  # (batch, L, dim=2560)

        result_esm2_650m = ESM2_650M(target_tokens.to("cpu"), repr_layers=[33], return_contacts=False)
        f1d_esm2_650M = result_esm2_650m["representations"][33][0, 1:-1, :].unsqueeze(0).to(device1)  # (1,L,1280)

        esm1v_single_1, esm1v_double_1 = gen_esm1v_logits(ESM1v_1, batch_converter_esm2, alphabet, seq_data)
        esm1v_single_2, esm1v_double_2 = gen_esm1v_logits(ESM1v_2, batch_converter_esm2, alphabet, seq_data)
        esm1v_single_3, esm1v_double_3 = gen_esm1v_logits(ESM1v_3, batch_converter_esm2, alphabet, seq_data)
        esm1v_single_4, esm1v_double_4 = gen_esm1v_logits(ESM1v_4, batch_converter_esm2, alphabet, seq_data)
        esm1v_single_5, esm1v_double_5 = gen_esm1v_logits(ESM1v_5, batch_converter_esm2, alphabet, seq_data)
        esm1v_single_logits = torch.cat([esm1v_single_1, esm1v_single_2, esm1v_single_3, esm1v_single_4, esm1v_single_5], dim=0).unsqueeze(0).to(device1)  # (1,5,L,20)
        esm1v_double_logits = torch.cat([esm1v_double_1, esm1v_double_2, esm1v_double_3, esm1v_double_4, esm1v_double_5], dim=0).unsqueeze(0).to(device1)  # (1,5,L,L,400)

    return (f1d_esm2_3B, f1d_esm2_650M, esm1v_single_logits, esm1v_double_logits, target_tokens[:, 1:-1], seq_data[0][1])
...
def threadTrain(spired_fitess_model, target_tokens, f1d_esm2_3B, f1d_esm2_650M, esm1v_single_logits, esm1v_double_logits, single_index, double_index, single_label, double_label, labelbatch, label_phi_psi, FLAGS, sample_idx, device1_G_E2S):
    # f1d_esm2_3B = (batch, 37, L, embed); f2dbatch = (batch, channel, L, L); labelbatch=(batch,4,L,L)

    L = esm1v_single_logits.shape[-2]
    print("f1d_esm2_3B", f1d_esm2_3B.shape)
    ...
    if len(single_index[0]) != 0:
        ...
        if len(double_index[0]) != 0:
            ...
            all_spearman_corr = spearman_corr(pred, label)
        else:
            ...
            all_spearman_corr = single_spearman_corr
    else:
        all_spearman_corr = torch.tensor(0.0).to(device=single_pred.device)
    ...
    return (single_pred, double_pred, Predxyz, PredCadistavg, Plddt, phi_psi_1D, soft_spearman_loss, all_spearman_corr, single_spearman_corr, double_spearman_corr, Loss_struct, Losse2e, LossCE, LossCA, loss_phi_psi_1D)
...
def threadValid(sample, spired_fitess_model, target_tokens, f1d_esm2_3B, f1d_esm2_650M, esm1v_single_logits, esm1v_double_logits, single_index, double_index, single_label, double_label, labelbatch, label_phi_psi, FLAGS, sample_idx, device1_G_E2S):
    ...
    if L < FLAGS.valid_length:
        print("L", L, "valid_length", "SPIRED on GPU")
        target_tokens, f1d_esm2_3B, f1d_esm2_650M, esm1v_single_logits, esm1v_double_logits, single_label, double_label, labelbatch, label_phi_psi = target_tokens.to(device=device1_G_E2S), f1d_esm2_3B.to(device=device1_G_E2S), f1d_esm2_650M.to(device=device1_G_E2S), esm1v_single_logits.to(device=device1_G_E2S), esm1v_double_logits.to(device=device1_G_E2S), single_label.to(device=device1_G_E2S), double_label.to(device=device1_G_E2S), labelbatch.to(device=device1_G_E2S), label_phi_psi.to(device=device1_G_E2S)
        spired_fitess_model = spired_fitess_model.to(device=device1_G_E2S)
        single_pred, double_pred, Predxyz, PredCadistavg, Plddt, ca, cb, omega, theta, phi, phi_psi_1D, f1d_cycle, f2d_cycle = spired_fitess_model(target_tokens, f1d_esm2_3B, f1d_esm2_650M, esm1v_single_logits, esm1v_double_logits)
    else:
        # transfer data to cpu
        target_tokens, f1d_esm2_3B, f1d_esm2_650M, esm1v_single_logits, esm1v_double_logits, single_label, double_label, labelbatch, label_phi_psi = target_tokens.to(device="cpu"), f1d_esm2_3B.to(device="cpu"), f1d_esm2_650M.to(device="cpu"), esm1v_single_logits.to(device="cpu"), esm1v_double_logits.to(device="cpu"), single_label.to(device="cpu"), double_label.to(device="cpu"), labelbatch.to(device="cpu"), label_phi_psi.to(device="cpu")
        spired_fitess_model = spired_fitess_model.to(device="cpu")
    ...
    if len(single_index[0]) != 0:
        ...
        if len(double_index[0]) != 0:
            ...
            soft_spearman_loss = Loss.spearman_loss(pred.unsqueeze(0), label.unsqueeze(0), FLAGS.regularization_strength, "kl")
            all_spearman_corr = spearman_corr(pred, label)
            # data.loc[index, 'pred'] = (double_mut_pred[second_mut_pos, first_mut_pos, double_mut_dict[second_mut_res + first_mut_res]].item() + double_mut_pred[first_mut_pos, second_mut_pos, double_mut_dict[first_mut_res + second_mut_res]].item()) / 2
            double_spearman_corr = spearman_corr(double_pred[0][double_index], double_label[0][double_index])
        else:
            print("calculate single mutations spearman corr")
            soft_spearman_loss = Loss.spearman_loss(single_pred[0][single_index].unsqueeze(0), single_label[0][single_index].unsqueeze(0), FLAGS.regularization_strength, "kl")
...
def threadTest(sample, spired_fitess_model, spired_fitess_model_cpu, target_tokens, f1d_esm2_3B, f1d_esm2_650M, esm1v_single_logits, esm1v_double_logits, FLAGS):
    # f1d_esm2_3B = (batch, L, 37, 2560)

    L = f1d_esm2_3B.shape[1]

    s0 = time.time()

    if L < FLAGS.valid_length:
        print("L", L, "SPIRED on GPU")
        single_pred, double_pred, Predxyz, PredCadistavg, Plddt, ca, cb, omega, theta, phi, phi_psi_1D, f1d_cycle, f2d_cycle = spired_fitess_model(target_tokens, f1d_esm2_3B, f1d_esm2_650M, esm1v_single_logits, esm1v_double_logits)
    else:
        # transfer data to cpu
        print("L", L, "SPIRED on CPU")
        target_tokens, f1d_esm2_3B, f1d_esm2_650M, esm1v_single_logits, esm1v_double_logits = target_tokens.to(device="cpu"), f1d_esm2_3B.to(device="cpu"), f1d_esm2_650M.to(device="cpu"), esm1v_single_logits.to(device="cpu"), esm1v_double_logits.to(device="cpu")

        single_pred, double_pred, Predxyz, PredCadistavg, Plddt, ca, cb, omega, theta, phi, phi_psi_1D, f1d_cycle, f2d_cycle = spired_fitess_model_cpu(target_tokens, f1d_esm2_3B, f1d_esm2_650M, esm1v_single_logits, esm1v_double_logits)

        # transfer model to gpu
        # spired_fitess_model = spired_fitess_model.to(device=spired_fitess_model.device)
...
def write_pdb(xyz, target_seq, coords_num, name, dir):
    ...
    # xyz = (N=8,L,3), N is the number of coordinate sets
    N, L, _ = xyz.shape
    if coords_num < N:
        N = coords_num
...
def train_loss_dict(LossDict, Predxyz, Losse2e, LossCE, LossCA, loss_phi_psi_1D):
    ...
    LossDict["train/sumloss"].append(Losse2e["optim"].item())  ## 必须.item() 否则会动态建图
