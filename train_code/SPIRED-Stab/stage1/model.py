...
class Model(torch.nn.Module):
    ...
    def forward(self, wt_data, mut_data, mut_pos):
        ...
        mut_pred = self.mlp((mut_embedding * mut_pos.unsqueeze(-1)).sum(1)).squeeze(-1)
        wt_pred = self.mlp((wt_embedding * mut_pos.unsqueeze(-1)).sum(1)).squeeze(-1)
        return mut_pred - wt_pred
