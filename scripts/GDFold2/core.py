...
class GradientDescent:
    ...
    def _theta2mat(self, theta):
        ...
        mat_x = torch.stack([onen, zeron, zeron, zeron, cos1, -sin1, zeron, sin1, cos1], dim=-1).view(self.npose, self.length, 3, 3)

        mat_y = torch.stack([cos2, zeron, sin2, zeron, onen, zeron, -sin2, zeron, cos2], dim=-1).view(self.npose, self.length, 3, 3)

        mat_z = torch.stack([cos3, -sin3, zeron, sin3, cos3, zeron, zeron, zeron, onen], dim=-1).view(self.npose, self.length, 3, 3)
    ...
    def _oxygen(self, mat, coords):
        ...
        oxygen = plane @ oxy + b
        oxygen_last = coords[0][:, -1] + torch.squeeze(mat[:, -1] @ (self.local[:, -1, :, 1] + oxy_last).unsqueeze(-1))
    ...
    def _hydrogen(self, mat, coords):
        ...
        hydrogen_first = coords[0][:, 0] + torch.squeeze(mat[:, 0] @ (self.local[:, 0, :, 2] + hyd_first).unsqueeze(-1))
    ...
    def _peptide_term(self, coords):
        ...
        v1 = torch.cross((coords[0][:, :-1] - coords[1][:, :-1]), (coords[2][:, 1:] - coords[1][:, :-1]))
        v2 = torch.cross((coords[1][:, :-1] - coords[2][:, 1:]), (coords[0][:, 1:] - coords[2][:, 1:]))
    ...
    def _dihedral_term(self, mat, coords):
        ...
        def last_C(phi):
            ...
            cos_angle = torch.sum(F.normalize(N - CA, dim=-1) * F.normalize(C - CA, dim=-1), dim=-1)
            rad_angle = torch.acos(cos_angle) - 1.5708
            v1 = 0.520 * pep_bond
            v2 = torch.sqrt(torch.square(pep_bond) - torch.square(v1) + self.epsilon)
            x = v2 * torch.cos(phi) * torch.cos(rad_angle[1:-1]) + (bond[1:-1] + v1) * cos_angle[1:-1]
            y = v2 * torch.cos(phi) * torch.sin(rad_angle[1:-1]) + (bond[1:-1] + v1) * torch.cos(rad_angle[1:-1])
            z = -torch.sign(phi) * torch.sqrt(torch.square(v2) - torch.square(v2 * torch.cos(phi)) + self.epsilon)
            return torch.stack([x, y, z], dim=-1)

        # infer the coordinates of the next N atom
        def next_N(psi):
            x = 0.449 * pep_bond + self.local[0, 1:-1, 0, 1]
            y = torch.sqrt(torch.square(pep_bond) - torch.square(0.449 * pep_bond) + self.epsilon) * torch.cos(psi)
            z = torch.sign(psi) * torch.sqrt(torch.square(pep_bond) - torch.square(0.449 * pep_bond) - torch.square(y) + self.epsilon)
            return torch.stack([x, y, z], dim=-1)
        ...
        self.dihedral_loss = torch.sum(loss(coords[1][:, :-2], label_C) + loss(coords[2][:, 2:], label_N), dim=-1)
